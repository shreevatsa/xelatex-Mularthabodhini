...
\makeatother
\usepackage{emptypage}  % Redefines \cleardoublepage such that the page possibly inserted before the next (odd-numbered) page has no page numbers or header.
\usepackage{relsize}  % "\relscale{1.1875}" is used to typeset verses in a larger font.
% \usepackage{multicol} % Not used; removing this doesn't affect the output.
\usepackage{pdfpages} % Used for \includepdf for DustJacket.pdf (also for pulling in eso-pic package I guess)
\usepackage{tocloft} % See \cftchapfont, ..., \cftbeforechapskip, ... \tableofcontents
\usepackage{calc}
\usepackage{fancyhdr}
\usepackage{array}
...
% \usepackage{setspace} % Not used; removing this doesn't affect the output.
\usepackage{lineno}
...
\usepackage{ragged2e}
\raggedbottom
% \usepackage[base]{babel} % Not used; removing this doesn't affect the output.
\usepackage{polyglossia}
%\usepackage{fontspec} % Already loaded by polyglossia.
% \usepackage{metalogo} % Not used; removing this doesn't affect the output.
%\setmainfont{Charis SIL} 
%\newfontfamily\nagarifont[Script=Devanagari]{NMCP}
\setmainlanguage{hindi}
\setotherlanguage{english}

\setmainfont[Scale=0.83, Script=Devanagari, Path = ./, Extension = .ttf,UprightFont = *-Regular, BoldItalicFont = *-BoldItalic, BoldFont = *-Bold, ItalicFont = *-Italic, Mapping=devanagaridigits]{Cambay}
...
\newfontfamily\devfont[Scale=0.83, Script=Devanagari, Path = ./, Extension = .ttf,UprightFont = *-Regular, BoldItalicFont = *-BoldItalic, BoldFont = *-Bold, ItalicFont = *-Italic, Mapping=devanagaridigits]{Cambay}
...
% Abbreviations, from file `ss.tex'
...
% Table of contents, using \tableofcontents
\fontsize{14}{15.75}\selectfont % Changing font size just for ToC
...
% Done with all \frontmatter now; switch to \mainmatter
\mainmatter
...
% A "संपादकीय" chapter, from files `sampadakiya_two.tex' and `sampadakiya.tex'
...
% A "प्राक्कथन" chapter, from file `prakkathana.tex'
\setcounter{footnote}{0}
\justifying
\phantomsection
...
% The start of the "main" part of the book
\phantomsection
\part*{श्रीभक्तमाल (मूलार्थबोधिनी टीका सहित)}

% पूर्वार्ध from file `purvardha.tex'
\setcounter{footnote}{0}
\justifying
\phantomsection
...
% उत्तरार्ध from file `uttarardha.tex'
\setcounter{footnote}{0}
\justifying
\phantomsection
...
\thispagestyle{empty}
\ % The empty page (could also have "\null" here)
\newpage
